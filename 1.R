...
## NOTE more data.table stuff in 3.R but might better revisit later
...
ggplot() +
    geom_map(data = world, map = world, aes(long, lat, map_id = region)) +
    coord_fixed(1.3) +
    geom_point(data = countries, aes(long, lat, size = rating), color = 'orange')

## fancy background from http://maps.stamen.com
map <- get_stamenmap(
    c(
        left = min(countries$long) * 0.995,
        right = max(countries$long) * 1.001,
        bottom = min(countries$lat) * 0.999,
        top = max(countries$lat)) * 1.001,
    maptype = 'toner-2011',
    zoom = 4)
...
## intro to https://www.autodesk.com/research/publications/same-stats-different-graphsw
