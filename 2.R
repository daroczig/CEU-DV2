...
for (i in 2:8) {
  ## NOTE the need to print .. otherwise pretty boring "animation"
...
## finish the datasaurus exercise from last week
library(data.table)
...
dt <- rbindlist(lapply(1:13, function(i)
  data.frame(
    ...
    dataset_id = i)))
## alternatively
dt <- rbindlist(lapply(seq(1, 26, by = 2), function(i)
  data.frame(
    x = df[, c(i), drop = TRUE],
...
## human-friendly dataset ids
dt <- rbindlist(lapply(seq(1, 26, by = 2), function(i) {
  data.frame(
    x = df[, c(i), drop = TRUE],
    y = df[, c(i+1), drop = TRUE],
    # sub('_x$', '', names(df)[i])
    dataset_id = substr(names(df)[i], 1, nchar(names(df)[i])-2))
}
))
...
## TODO bug: overlapping title and subtitle?! not sure ... let's move it down to "x"
ggplot(datasaurus_dozen, aes(x, y)) +
  geom_point() + geom_smooth(method = 'lm') +
  transition_states(dataset) +
  labs(
    x = "{closest_state}", y = "",
    subtitle = '{subtitle(subset(datasaurus_dozen, dataset == closest_state))}') +
  theme(plot.margin = ggplot2::margin(1, 0, 0, 0, "cm"))
...
g <- ggplot(dt, aes(order, delay)) +
  ...
  ## NOTE labels for negative values
...
## quite too much .. maybe filter for top10
dt[order<=10]
